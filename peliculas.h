/*Variable para saber el numero de peliculas*/
int NumeroPeliculas = 0;
//estructuras de datos de las peliculas, segun los campos que se necesita
struct Peliculas{
    string Nombre;
...
void LeerPeliculas(){
    ...
    while (!Archivo.eof()) {
        ...
        if (!Archivo.eof()){
            ...
            /*Recorro los registros del archivo, cada ocho espacios separados por pipe es una tupla*/
            for(int i = 0; i < Contenido.size(); i=i+8){
                if(NumeroPeliculas < maximo){
                    ...
                    pelicula[NumeroPeliculas].Precio          = stof(Contenido[i+7].c_str());;
                    NumeroPeliculas++;
                }
            }
...
void ConfirmarPeliculas(){
    ...
    for(int i = 0; i < NumeroPeliculas; i++){
        ...
        Archivo << pelicula[i].AnioProduccion << "|";
        Archivo << pelicula[i].Precio << "|"<< endl;
    }
    /*Cierro el archivo*/
    Archivo.close();
...
void AgregarPeliculas(){
    /*Agregar peliculas, segun campos ya declarados en el iterador*/
    ...
    cout << "Agregar precio para el publico "<< "\n";
    cin >> pelicula[NumeroPeliculas].Precio;
...
void VerPeliculas(){
    ...
    for (int i=0; i < NumeroPeliculas; i++){
        ...
        << "Anio Produccion: " << pelicula[i].AnioProduccion << "\n"
        << "Anio Produccion: " << pelicula[i].Precio << endl << endl;
    }
    Limpiar();
}
